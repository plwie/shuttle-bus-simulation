...
func (g *Graph) AddEdge(parent, child *BusStop, cost, speedlimit int) {
 edge := &Edge{
  Parent: parent,
  Child:  child,
  Cost:   cost,
...
// 	return p
// }
func (g *Graph) GetDistance(parent *BusStop, child *BusStop) (distance int) {
	for _, edge := range g.Edges {
    ...
    if child == nil {
        ...
        for _, edge := range g.Edges {
            if edge.Parent == parent && edge.Child == child  {
                temp := rand.Intn(65) + len(randomCar)  
                edge.Density = float64(temp/2)
				// edge.SpeedLimit = int(edge.Density*10)
                edge.Level = int(math.Floor(edge.Density/10))
            }
        }
    }
...
// func (g *Graph) calulateDistance(source string) float64 {
// 	edgeLen := len(g.Edges)
// 	var dis float64
// 	for i := 0; i < edgeLen; i++ {
// 	 if source != g.Edges[i].Parent.Name {
// 	  continue
// 	 } else {
// 	  dis = float64(g.Edges[i].Cost)
// 	 }
// 	}
// 	return dis
//    }

// func (g*Graph) GetDistance(stopList *[]*BusStop, buildingInputJson string) (){
...
//             if building.Source == source && building.Destination == destination{
// 				return distance
// 			}
// 		}
// 	}
...
func (g *Graph) GenerateBuildingBusStop(stopList *[]*BusStop, buildingInputJson string) (){
    ...
    for _, building := range buildingInput.BusStopList{
        ...
        // Add edge
        g.AddEdge(initBuildingList[building.Source], initBuildingList[building.Destination], building.Distance, building.SpeedLimit)
	    g.AddEdge(initBuildingList[building.Destination], initBuildingList[building.Source], building.Distance, building.SpeedLimit)
		...
		// g.GetSpeed(initBuildingList[building.Source], initBuildingList[building.Destination], building.Distance, building.SpeedLimit)
	    // g.GetSpeed(initBuildingList[building.Destination], initBuildingList[building.Source], building.Distance, building.SpeedLimit)
		// g.GetDistance(building.Distance)
    }	
}